...
class MitmMock:
    ...
    def eval_preppy_value(self, value):
        if type(value) == str:
            return preppy.getModule('mm_part',
                                    sourcetext=value,
                                    source_extension=None).getOutput({})
        if type(value) != str and isinstance(value, Iterable):
            return [self.eval_preppy_value(v) for v in value]
        return value

    def eval_preppy(self, obj: dict):
        return {self.eval_preppy_value(k): self.eval_preppy_value(v) for k, v in obj.items()}

    def request(self, flow: HTTPFlow):
        saved_resp = self.responses[self.ptr]
        headers = Headers()
        for k, v in saved_resp['headers']:
            headers[self.eval_preppy_value(k)] = self.eval_preppy_value(v)
